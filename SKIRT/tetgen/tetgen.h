...
#define fastlookup(pool, index) \
    ...
    int insertpoint_cdt(point, triface*, face*, face*, insertvertexflags*, arraypool*, arraypool*, arraypool*,
    ...
    int checkflipeligibility(int fliptype, point, point, point, point, point, int level, int edgepivot,
                             flipconstraints* fc);
...
inline void terminatetetgen(tetgenmesh* m, int x)
{
    ...
    switch (x)
    {
        ...
        case 200: printf("Boundary contains Steiner points (-YY option). Program stopped.\n"); break;
    }  // switch (x)
    exit(x);
#endif  // #ifdef TETLIBRARY
}
...
{
    return (pointtype(pt) == FREESEGVERTEX) || (pointtype(pt) == FREEFACETVERTEX) || (pointtype(pt) == FREEVOLVERTEX);
}
