...
double Band::meanSpecificLuminosity(const Array& lambdav, const Array& pv) const
{
    ...
    for (size_t i=0; i!=_size; ++i) if (range.contains(_lambdav[i])) newlambdav.push_back(_lambdav[i]);
    for (double lambda : lambdav) if (range.contains(lambda)) newlambdav.push_back(lambda);
    NR::sort(newlambdav);
