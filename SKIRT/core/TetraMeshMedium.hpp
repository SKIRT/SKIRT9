...
#ifndef TETRAMESHMEDIUM_HPP
#define TETRAMESHMEDIUM_HPP
...
/** A TetraMeshMedium instance represents a transfer medium with a spatial density distribution
    described by a list of sites generating a Tetra tesselation of a cubodail domain. The data is
    ...
    The first three columns are the \f$x\f$, \f$y\f$ and \f$z\f$ coordinates of the Tetra site
    (i.e. the location defining a particular Tetra cell).
    ...
    <b>Avoiding construction of the Tetra tessellation</b>

    The algorithm used by the TetraMeshSnapshot class for constructing Tetra tessellations
    sometimes fails (for example, when generating sites are too close to each other). In those
    cases, it can be desirable to forego the construction of the Tetra tessellation and instead
    ...
    case is when the medium density distribution defined on the Tetra grid is resampled to an
    octree grid to perform radiative transfer. However, the octree subdivision algorithm requires
    the total mass of the medium in addition to the density samples. Without the full Tetra
    ...
    configuration allows it (i.e. the full Tetra tessellation is not needed for other purposes),
    the TetraMeshSnapshot class will use the information in these two columns to calculate the
    cell volumes and the total medium mass, and it will forego construction of the Tetra
    tessellation. */
class TetraMeshMedium : public MeshMedium, public TetraMeshInterface
{
    ITEM_CONCRETE(TetraMeshMedium, MeshMedium, "a transfer medium imported from data represented on a Tetra mesh")
        ATTRIBUTE_TYPE_INSERT(TetraMeshMedium, "TetraMeshInterface")
    ITEM_END()

    //============= Construction - Setup - Destruction =============

protected:
    /** This function constructs a new TetraMeshSnapshot object, calls its open() function,
...
protected:
    /** This function implements the TetraMeshInterface interface. It returns a pointer to the
        Tetra mesh snapshot maintained by this geometry. */
    TetraMeshSnapshot* tetraMesh() const override;

    //===================== Data members ====================

private:
    // an extra pointer to our snapshot used to implement TetraMeshInterface (ownership is passed to base class)
    TetraMeshSnapshot* _tetraMeshSnapshot{nullptr};
};
