...
#ifndef SPHERICALBACKGROUNDSOURCE_HPP
#define SPHERICALBACKGROUNDSOURCE_HPP

#include "CenteredSource.hpp"

////////////////////////////////////////////////////////////////////

/** The SphericalBackgroundSource class represents the surface of a sphere of radius
    \f$r_{\text{bg}}\f$ from which radiation escapes in the inward direction. The emissivity of
    this source is anisotropic. There is no radiation outwards and the intensity is proportional to
    \f$\cos\theta'\f$ in the inward hemisphere, where \f$\theta'\f$ is the angle between the
    direction and the normal on the surface. */
class SphericalBackgroundSource : public CenteredSource
{
...
public:
    ...
    /** This function causes the photon packet \em pp to be launched from the source using the
        ...
        background surface by drawing a random direction \f${\bf{u}}\f$ on the unit sphere and
        transforming it to take into account the background radius and center position. The
        direction \f${\bf{u}}\f$ actually corresponds to the outward normal to the background
        surface at the position \f${\bf{r}}\f$.
        ...
        normal \f${\bf{u}}\f$. In a spherical coordinate system with the Z' axis normal to the
        surface at the position \f${\bf{r}}\f$, the probability distribution is given by \f[
        p(\theta',\varphi')\, {\text{d}}\theta'\, {\text{d}}\varphi' = -\frac{1}{\pi}\cos\theta'
        ...
        \f] Once these values have been determined, we need to determine the coordinates of the
        vector \f${\bf{k}}\f$ in the standard coordinate system. The link between the standard XYZ
