...
void AllSkyProjectionForm::writeQuantity(const ProbeFormBridge* bridge) const
{
    ...
    FITSInOut::write(bridge->probe(), "all-sky-projected " + bridge->description(), bridge->prefix(), vvv.data(),
                     bridge->projectedUnit(), Nx, Ny, units->oposangle(-2 * M_PI / Nx), units->oposangle(M_PI / Ny), 0.,
                     0., units->uposangle(), bridge->axis(), bridge->axisUnit());
}
