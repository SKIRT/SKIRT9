...
/** DefaultmediaDensityCutsProbe outputs FITS files with cuts through the theoretical media density
    ...
    The difference between the theoretical density maps (named <tt>prefix_probe_MM_t_XX.fits</tt>)
    and the grid-discretized density maps (named <tt>prefix_probe_MM_g_XX.fits</tt>) is the
    following: the theoretical density is the density represented by the media system, i.e.\ the
    true density that would correspond to an infinitely fine spatial grid. The grid-discretized
    density maps on the other hand give the value of the density as read from the finite-resolution
    spatial grid in the simulation. A comparison of both sets of maps can reveal whether the
    configured spatial grid is suitable (in the ideal case, there would be no difference between
    both sets of maps). */
class DefaultMediaDensityCutsProbe : public Probe
{
    ITEM_CONCRETE(DefaultMediaDensityCutsProbe, Probe, "cuts of the media density along the coordinate axes")
    ITEM_END()
