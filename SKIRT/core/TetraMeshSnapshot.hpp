...
public:
    /** This function reads the snapshot data from the input file, honoring the options set through
        ...
        The function calls the private buildMesh() function to build the Tetra mesh based on the
...
public:
    /** This function sets the extent of the spatial domain for the Tetra mesh. When using the
...
private:
    ...
    /** Given a list of generating sites (represented as partially initialized Cell
        ...
        review 41.4, pp 637-676; Dobbels 2017, master thesis). An intermediate Tetra tessellation
...
public:
    /** This function outputs grid plot files as described for the SpatialGridPlotProbe. The
        function reconstructs the Tetra tesselation in order to produce the coordinates of the
        Tetra cell vertices. */
...
public:
    ...
    /** This function returns the bounding box (enclosing cuboid lined up with the coordinate axes)
        of the Tetra cell with index \em m. If the index is out of range, the behavior is
        undefined. */
    ...
    /** This function returns the cell index \f$0\le m \le N_{cells}-1\f$ for the cell containing
        ...
        returns -1. By definition of a Tetra tesselation, the closest site position determines
        the Tetra cell containing the specified point.
        ...
        of the specified point, which provides a list of Tetra cells possibly overlapping the
...
public:
    /** This function creates and hands over ownership of a path segment generator appropriate for
        ...
        the path direction with the current cell's boundary. By the nature of Tetra cells, this
