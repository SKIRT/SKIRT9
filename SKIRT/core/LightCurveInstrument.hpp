...
/** A LightCurveInstrument object represents a distant instrument that records the spatially integrated
    flux density for each time and outputs a light curve text column file.

    Note that this instrument must be used when the simulation mode is set to \em ExtinctionOnly; using it in other
    simulation modes will lead to an error.
