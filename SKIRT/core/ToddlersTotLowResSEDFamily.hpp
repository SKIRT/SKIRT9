...
#ifndef TODDLERSTOTLOWRESSEDFAMILY_HPP
#define TODDLERSTOTLOWRESSEDFAMILY_HPP
...
/** An instance of the ToddlersTotLowResSEDFamily class represents the family of 
    ...
    This class uses the low resolution spectra (R=300) to represent the total SED 
    (line + continuum).
...
class ToddlersTotLowResSEDFamily : public SEDFamily
{
    ITEM_CONCRETE(ToddlersTotLowResSEDFamily, SEDFamily, "a Toddlers SED family for low resolution total emission (lines + continuum, R=300) from Hii regions")
        ATTRIBUTE_TYPE_DISPLAYED_IF(ToddlersTotLowResSEDFamily, "Level2")
    ITEM_END()
...
public:
    /** This constructor can be invoked programmatically by classes that use a hard-coded SED
        ...
        */
    explicit ToddlersTotLowResSEDFamily(SimulationItem* parent);
