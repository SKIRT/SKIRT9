...
/** An instance of the BruzualCharlotSEDFamily class represents the family of Bruzual & Charlot
    ...
    available for two assumed initial mass functions (Chabrier en Salpeter). Each of those families
    can be loaded in two wavelength resolution versions, with respectively 1221 and 6900 wavelength
    points. The low-resolution version uses less resources and is sufficient for most purposes.
    However, if the model under study zooms in on narrow wavelength ranges, the high-resolution
