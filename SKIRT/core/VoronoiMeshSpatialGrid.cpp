...
namespace
{
    ...
    vector<Vec> sampleMedia(const vector<Medium*>& media, const vector<double>& weights,
                            const Box& extent, int numSites)
    {
    ...
    case Policy::DustDensity:
        {
            ...
            for (auto medium : ms->media()) if (medium->mix()->isDust()) media.push_back(medium);
            for (auto medium : media) weights.push_back(medium->mass());
            _mesh = new VoronoiMeshSnapshot(this, extent(),
                                            sampleMedia(media, weights, extent(), _numSites), _relaxSites);
            break;
        }
    case Policy::ElectronDensity:
        {
            ...
            vector<double> weights;
            auto ms = find<MediumSystem>();
            for (auto medium : ms->media()) if (medium->mix()->isElectrons()) media.push_back(medium);
    ...
    case Policy::GasDensity:
        {
            ...
            vector<double> weights;
            auto ms = find<MediumSystem>();
            for (auto medium : ms->media()) if (medium->mix()->isGas()) media.push_back(medium);
            for (auto medium : media) weights.push_back(medium->number());
            _mesh = new VoronoiMeshSnapshot(this, extent(),
                                            sampleMedia(media, weights, extent(), _numSites), _relaxSites);
            break;
