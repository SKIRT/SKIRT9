...
/** The XRayAtomicGasMix class describes the material properties related to photo-absorption and
    ...
    wavelength grids to resolve specific features of interest. To study fluorescence lines
    seperated from the background continuum, consider the scattered flux component generated by an
    instrument configured to record flux components seperately.

    <b>\em Crucially \em important:</b> in the \c PhotonPacketOptions section, set the advanced
    property \em minWeightReduction to a value of \c 1e10. With the default value of \c 1e4,
    essentially all emitted fluorescence packets are killed before having a chance to register in
    the instruments.
