...
protected:
    /** This function calculates some frequently used values. The normalization
...
public:
    ...
    /** This function generates a random position from the torus geometry, by
        ...
        For the radial coordinate, the appropriate probability distribution is \f$ p(r)\,{\text{d}}r \propto
        r^{2-p}\,{\text{d}}r \f$. A random radius is generated by picking a new uniform deviate
        \f${\cal{X}}\f$, and solving the equation \f[ {\cal{X}} = \int_0^r p(r')\, {\text{d}}r'
        \f] for \f$r\f$. For \f$p\ne3\f$ we find \f[ {\cal{X}} = \frac{r^{3-p}-r_{\text{min}}^{3-p}}
        {r_{\text{max}}^{3-p}-r_{\text{min}}^{3-p}}. \f] Inverting this results in \f[ r = \left[
        (1-{\cal{X}})\,r_{\text{min}}^{3-p} + {\cal{X}}\,r_{\text{max}}^{3-p} \right]^{\frac{1}{3-p}}. \f]
        For \f$p=3\f$ this expression does not hold, and for \f$p\approx3\f$ it breaks down
        numerically. So for \f$p\approx3\f$ we can write the general expression \f[ r =
        {\text{gexp}}_{p-2} \Big[ {\text{gln}}_{p-2}\, r_{\text{min}} + {\cal{X}}\,(
