...
namespace LyaUtils
{
    ...
    /** This function returns the emission wavelength range for which the photon packet life cycle
        ...
        section given in the class header as \f[ \tau^* = \mathcal{N}^* \,\sigma_{\alpha,0}
        \,H(a_\mathrm{v},x^*) \f] where \f$x^*\f$ is the dimensionless frequency shift to be
        calculated. Assuming that the required maximum optical depth \f$\tau^*\f$ is sufficiently
        low for \f$x^*\f$ to be in the wings of the Voigt profile, we can use the corresponding
        approximation \f$ H(a_\mathrm{v},x^*) \approx a_\mathrm{v} / \sqrt{\pi}x^{*2} \f$. After
        substituting the definitions of the other quantities as given in the class header, we
        obtain \f[ v_\mathrm{p,Voigt}^* = \sqrt{\frac{3 A_\alpha^2 \lambda_\alpha^4}{64 \pi^3} \,
        \frac{\mathcal{N}^*}{\tau^*}}. \f] Interestingly, the dependence on temperature (hidden in
        \f$\sigma_{\alpha,0}\f$, \f$a_\mathrm{v}\f$ and \f$x^*\f$) has canceled out of this
        equation.

        The current implementation uses \f$\tau^*=10^{-3}\f$.
