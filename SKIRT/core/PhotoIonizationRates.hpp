...
/** The PhotoIonizationRates namespace provides rate coefficients for recombination (radiative and
...
namespace PhotoIonizationRates
{
    ...
    inline double alphaDRNeIX(double T)
    {
    ...
    inline double alphaDRFeXVII(double T)
    {
        return pow(T, -1.5)
               * (0.0006342 * exp(-2.77e6 / T) + 0.0835 * exp(-3.978e6 / T) + 1.045 * exp(-7.052e6 / T)
                  + 0.3663 * exp(-1.3e7 / T) - 0.03955 * exp(-3.579e7 / T));
    }
