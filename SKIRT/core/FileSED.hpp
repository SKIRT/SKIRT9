...
/** A FileSED object represents a spectral energy distribution that is loaded from an input file.
    ...
    columns in the file are ignored. The luminosity outside the range indicated by the first and
    the last wavelength in the file is considered to be zero.

    The wavelengths are by default given in micron (the units can be overridden by column header
    info in the file) and must listed be in increasing order. The specific luminosity values are by
    default given in per-wavelength units. Again, this can be overridden by column header info in
    the file to use per-frequency or neutral units. Other than this, the scaling of the values is
    arbitrary because the %SED will be normalized after being loaded. However, the input procedure
