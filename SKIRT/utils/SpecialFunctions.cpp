...
double SpecialFunctions::incompleteGamma(double a, double x)
{
    ...
    if (x < 0.0 || a <= 0.0)
        throw FATALERROR("Invalid arguments in routine SpecialFunctions::incompletegamma");
    ...
    else
    {
        ...
        if (i > ITMAX)
            throw FATALERROR("a too large, ITMAX too small in SpecialFunctions::incompletegamma");
...
double SpecialFunctions::rd(double x, double y, double z)
{
    ...
    if (x<0.0 || y<0.0 || x+y<TINY || z<TINY || x>BIG || y>BIG || z>BIG) throw FATALERROR("Invalid argument(s)");
    xt = x;
    yt = y;
    zt = z;
    sum = 0.0;
...
double SpecialFunctions::rj(double x, double y, double z, double p)
{
    ...
    double a=0.0, alamb, alpha, ans, ave, b=0.0, beta, delp, delx, dely, delz,
            ea, eb, ec, ed, ee, fac, pt, rcx=0.0, rho, sqrtx, sqrty, sqrtz, sum,
            tau, xt, yt, zt;

    if (x<0.0 || y<0.0 || z<0.0 || x+y<TINY || x+z<TINY || y+z<TINY || fabs(p)<TINY ||
        x>BIG || y>BIG || z>BIG || fabs(p)>BIG) throw FATALERROR("Invalid argument(s)");
    sum = 0.0;
    fac = 1.0;
    if (p > 0.0)
    {
    ...
    do {
        ...
        delz = (ave-zt)/ave;
        delp = (ave-pt)/ave;
    } while (fabs(delx)>ERRTOL || fabs(dely)>ERRTOL ||
             fabs(delz)>ERRTOL || fabs(delp)>ERRTOL );
...
double SpecialFunctions::LambertW1(double z)
{
    ...
    const double em1=0.3678794411714423215955237701614608;
    static const double c[12] = {-1.0, 2.331643981597124203363536062168,
...
double SpecialFunctions::DebyeD(int n, double x)
{
    ...
    if (x >= 3.)
    {
        ...
        // od:
        static const double nzetan[] =
                             { 0., 1.64493406684822643647241516665e+00, 2.40411380631918857079947632302e+00,
    ...
    else
    {
        ...
        // od;
        static const double koeff[] =
                           { 0., 1.289868133696452872944830333292e+00, 1.646464674222763830320073930823e-01,
...
double SpecialFunctions::gexp(double p, double x)
{
    ...
    {
        double x2 = x*x;
        return exp(x) * (1.0 - 0.5*x2*q + 1.0/24.0*x*x2*(8.0+3.0*x)*q*q
...
double SpecialFunctions::lnmean(double x1, double x2)
{
    ...
    if (x < 1e-3)
    {
        return x1 / (1. - 1./2.*x + 1./3.*x*x - 1./4.*x*x*x + 1./5.*x*x*x*x - 1./6.*x*x*x*x*x );
    }
    else
    {
