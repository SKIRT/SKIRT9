...
namespace CompileTimeUtils
{
    ...
    template<typename... Ts> constexpr bool all(bool b, Ts... bs)
    {
        return b && all(bs...);
    }

    // returns true if the parameter pack has only integral arguments
    template<typename... Ts> inline constexpr bool isIntegralArgList()
    {
    ...
    template<typename... Ts> inline constexpr bool isNumericArgList()
    {
