...
int StringUtils::indexOf(const vector<string>& list, string find)
{
    ...
    if (it != list.cend()) return static_cast<int>(it-list.cbegin());
    else return -1;
}
...
{
    if (!filename.empty() &&
        !endsWith(StringUtils::toLower(filename), "." + toLower(extension)))
    {
        filename += "." + extension;
    }
    return filename;
...
bool StringUtils::isValidBool(string value)
{
    ...
    if (value == "true"  || value == "t" || value == "yes" || value == "y" || value == "1" ||
        value == "false" || value == "f" || value == "no"  || value == "n" || value == "0") return true;
    return false;
}
...
string StringUtils::toString(double value)
{
    ...
    for (auto it = result.crbegin(); it != result.crend() ; ++it, ++zeroes) if (*it != '0') break;
    if (zeroes > 3)
    {
...
string StringUtils::toMemSizeString(size_t value)
{
    ...
    return toString(static_cast<double>(value)/static_cast<double>(factor), 'g', 3)
            + ' ' + prefixes[index] + 'B';
}
