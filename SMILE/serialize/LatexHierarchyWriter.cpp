...
namespace
{
    ...
    private:
        ...
        const vector<std::pair<string,string>> _replText =
        {
            ...
            {"π","$\\pi$"},{"ρ","$\\rho$"},{"σ","$\\sigma$"},{"τ","$\\tau$"},{"ϕ","$\\phi$"},
            {"φ","$\\varphi$"},{"χ","$\\chi$"},{"ψ","$\\psi$"},{"ω","$\\omega$"},{"Γ","$\\Gamma$"},
            {"Δ","$\\Delta$"},{"Θ","$\\Theta$"},{"Λ","$\\Lambda$"},{"Π","$\\Pi$"},{"Σ","$\\Sigma$"},
            {"Φ","$\\Phi$"},{"Ψ","$\\Psi$"},{"Ω","$\\Omega$"},
            // accented characters
    ...
    public:
        ...
        string fromTextUpp(string str) const
        {
            return fromText(StringUtils::toUpperFirst(str));
        }
        ...
        string fromDouble(string str) const
        {
            ...
            if (segments.size()>1)
            {
                ...
                for (string& token : tokens)        // writable reference
                {
                    ...
                    if (_replUnit.count(token)) token = _replUnit.at(token);
                    else
                    {
...
namespace
{
    ...
    public:
            ...
            : _schema(schema), _out(System::ofstream(filePath))
        {
            ...
            _out << "Generated by " << tex.fromText(!producer.empty() ? producer : "SMILE")
                 << " on " << System::timestamp() << "\\\\\n";
        ...
        void visitPropertyHandler(IntPropertyHandler* handler) override
        {
            ...
            writeIndent();
            _out << tex.fromTextUpp(handler->title())
                 << ": $" << StringUtils::toString(handler->value()) << "$\\\\\n";
        }
        ...
        void visitPropertyHandler(EnumPropertyHandler* handler) override
        {
            ...
            writeIndent();
            _out << tex.fromTextUpp(handler->title())
                 << ": " << tex.fromText(handler->titleForValue()) << "\\\\\n";
        }
        ...
        void visitPropertyHandler(DoublePropertyHandler* handler) override
        {
            ...
            writeIndent();
            _out << tex.fromTextUpp(handler->title())
                 << ": " << tex.fromDouble(handler->toString(handler->value())) << "\\\\\n";
        }
        ...
        void visitPropertyHandler(DoubleListPropertyHandler* handler) override
        {
            ...
            writeIndent();
            _out << tex.fromTextUpp(handler->title())
                 << ": " << tex.fromDoubleList(handler->toString(handler->value())) << "\\\\\n";
        }
        ...
        void visitPropertyHandler(ItemListPropertyHandler* handler) override
        {
            ...
            for (Item* item : handler->value())
            {
                ...
                writeIndent();
                _out << "Item \\#" << std::to_string(index) << " in " << tex.fromText(handler->title()) << " list: "
                     << tex.fromText(_schema->title(item->type())) << "\\\\\n";
